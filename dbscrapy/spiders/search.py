...
class SearchSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for keyword in self.keyword_list:
            if not self.settings.get('REGION') or '全部' in self.settings.get(
                ...
                # 0 https://s.weibo.com/weibo?q=人工智能&typeall=1&timescope=custom:{2015-03-01}:{2020-03-01}
    ...
    def parse_by_hour(self, response):
        ...
        if is_empty:
            ...
            for region in self.regions.values():
                # https://s.weibo.com/weibo?q=人工智能}&region=custom::1000
    ...
    def parse_page(self, response):
        ...
        if is_empty:
            ...
            # 把解析后页面的整个页面传给parse_weibo， parse_weibo会分解每个card 然后yield每个DbscrapyItem对象给管道
            for weibo in self.parse_weibo(response):
                self.check_environment()
                yield weibo
            next_url = response.xpath(
